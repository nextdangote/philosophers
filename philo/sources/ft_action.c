...
void ft_eat(t_philo *philo, int i)
{
    ...
    if ((current_time.tv_usec - philo[i].last_meal_time) >= philo->time_to_die)
        ft_die(philo, i);
    //take r_fork and l_fork
    pthread_mutex_lock(&philo[i].right_fork);
    ft_print_action("has taken right fork\n", philo, i);
    pthread_mutex_lock(&philo[i].left_fork);
    ft_print_action("has taken left fork\n", philo, i);
    usleep(philo[i].time_to_sleep);
    pthread_mutex_lock(&philo[i].lock);
    //update last meal time
    philo[i].last_meal_time = current_time.tv_usec;
